program main
    ...
    bitvector = random_bitvector(nr_values, status)
    if (status /= 0) then
        write (unit=error_unit, fmt='(A, I0)') 'error: can not allocate bitvector of size ', nr_values
        stop 3
    end if
...
contains
    ...
    subroutine get_arguments(nr_values)
        ...
        if (nr_values < 0) then
            write (unit=error_unit, fmt='(A)') 'error: number of values should be positive'
            stop 2
        end if
